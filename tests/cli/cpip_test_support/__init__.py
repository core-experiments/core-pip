...
class CpipTestEnvironment(TestFileEnvironment):
    ...
    def __init__(
        ...
        *args: Any,
        ...
        cpip_expect_warning: bool = False,
    ...
    def run(
        ...
        **kw: Any,
        ...
        return TestCpipResult(result, verbose=self.verbose)
    ...
    def cpip_install_local(
        ...
        **kwargs: Any,
        ...
        return self.cpip(*cmd, **kwargs)

    def cpip_install_local_report(
        self,
        *args: StrPath,
        find_links: StrPath | list[StrPath] = pathlib.Path(DATA_DIR, "packages"),
        **kwargs: Any,
    ...
    def easy_install(self, *args: str, **kwargs: Any) -> TestCpipResult:
...
def assert_all_changes(
    start_state: FilesState | TestCpipResult,
    ...
    scripttest.[FoundFile|FoundDir] objects or a TestCpipResult whose
    files_before we'll test.  end_state is either a similar dict or a
    TestCpipResult whose files_after we'll test.
    ...
    end_files = end_state
    if isinstance(start_state, TestCpipResult):
...
def vcs_add(
    ...
    if vcs == "git":
        ...
        subprocess.check_call(
            ["bzr", "whoami", "cpip <distutils-sig@python.org>"],
            cwd=os.fspath(version_pkg_path),
        )
        subprocess.check_call(
            [
...
def create_svn_repo(repo_path: pathlib.Path, version_pkg_path: StrPath) -> str:
    ...
    subprocess.check_call(
        [
            ...
            repo_url,
            "-m",
            "Initial import of cpip-test-package",
        ],
...
class InMemoryCpip:
    def cpip(self, *args: str | pathlib.Path) -> InMemoryCpipResult:
        ...
        return InMemoryCpipResult(returncode, stdout.getvalue())
