...
if TYPE_CHECKING:
    from cpip.resolution.engine.context import ConflictContext
...
class ConflictLearning:
                ...
                (
                    (
                        ...
                        self.assignment_levels.get(
                            term,
                            decision_level if term == candidate_term else 0,
                        ),
                    )
                    for term in terms
                ),
    ...
    def package_id_internal(self: ConflictContext, name: str) -> int:
    ...
    def bump_conflict_activity(self: ConflictContext, *names: str) -> None:
    ...
    def compact_learned_incompatibilities(self: ConflictContext) -> None:
    ...
    def domain_version_mask(self: ConflictContext, domain: PackageDomain) -> int | None:
