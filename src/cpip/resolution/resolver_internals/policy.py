...
from cpip.resolution.resolver_internals.validation import ResolverValidationOperations
...
class ResolverPolicyOperations:
    """Candidate policy and resolver diagnostic operations."""

    def upgrade_allowed_for(self: ResolverContext, name: str) -> bool:
        if not self.upgrade:
            return False
    ...
    def validate_candidate_policy(
        self: ResolverContext, candidate: WheelCandidate
    ) -> None:
    ...
    def warn_missing_candidate_extras(
        self: ResolverContext, requirement: Requirement, candidate: WheelCandidate
    ) -> None:
...
class ResolverChecks(ResolverPolicyOperations, ResolverValidationOperations):
