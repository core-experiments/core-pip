...
from cpip.resolution.fast_wheelhouse.metadata import (
    load_candidate,
    parse_version,
...
def preflight_exact_dependencies(
    ...
    for dependency in candidate.dependencies:
        ...
        if (
            ...
            cache_preflight_result(
                shared_preflight_cache, exact_index, candidate, extras, True
            )
            return True
        ...
        if not entries:
            ...
            cache_preflight_result(
                shared_preflight_cache, exact_index, candidate, extras, False
            )
            return False
        if len(entries) != 1:
            preflight_cache[cache_key] = True
