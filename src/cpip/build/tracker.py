...
def get_build_tracker() -> Generator[BuildTracker, None, None]:
    root = os.environ.get("CPIP_BUILD_TRACKER")
    if root is not None:
    ...
    with TempDirectory(kind="build-tracker") as temporary_directory:
        ...
        previous = os.environ.get("CPIP_BUILD_TRACKER")
        os.environ["CPIP_BUILD_TRACKER"] = root
        try:
            ...
            if previous is None:
                del os.environ["CPIP_BUILD_TRACKER"]
            else:
                os.environ["CPIP_BUILD_TRACKER"] = previous
