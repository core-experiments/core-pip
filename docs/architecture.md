...
cpip uses independent per-purpose caches under one root rather than a single
database. The storage shape follows the data: small validated maps use atomic
marshal snapshots; the two incremental key/value stores use SQLite; large
immutable bytes use content-addressed files; reusable installs use cloneable
directory trees.

Every persisted cache lives under the `v<CACHE_VERSION>` directory of the
cache root (`core/appdirs.py:versioned_cache_dir`, currently `v0/`), which is
what `resolve_cache_dir`/`configured_cache_dir` hand to every writer. That one
directory is the cache system's version: no cache carries a version in its
name or payload, bumping `core/utils.py:CACHE_VERSION` makes every existing
cache a miss at once, there is no migration code, and `cpip cache purge`
removes every `v*` directory without knowing the individual stores.

| Owner | Storage under `v0/` | Contents and validity |
| --- | --- | --- |
| `network/cache.py` | `http/` | HTTP metadata/body pairs under hashed keys; missing or partial pairs are misses |
| `index/catalog_cache.py` | records in the HTTP cache | parsed Simple API links keyed by source URL; the release summary stores each version as `Version.to_wire()` (`public`, `release`, ordering key) and one that fails its checksum or shape check is recompiled from the text-only catalog |
| `index/artifact_cache.py` | `artifacts/` | immutable bodies by SHA-256 plus normalized-URL receipts and expected-hash validation |
| `index/candidate_cache.py` | `wheels/` | wheels built from source, keyed by stable source identity |
| `index/metadata_cache.py` | `metadata.sqlite` | parsed local wheel headers keyed by absolute path, size, and modification time |
| `index/candidate_metadata_cache.py` | `candidate-metadata.sqlite` | dependency metadata safe to reuse during resolution, stored as text and reparsed through the `core` intern tables |
| `index/release_facts_cache.py` | `release-facts.marshal` | deterministic release-level rejection reasons |
| `cli/fast.py` | `fast-lock-plan/` | rendered lock output keyed by interpreter, requirements and find-links |
| `cli/fast_install.py` (`FastInstallMetadataCache`) | `fast-install-<interpreter>.marshal` and `fast-install-trees/` | narrow local plans, wheel metadata, and cloneable completed targets |
| `install/wheel_archive_cache.py` | `archive-<interpreter>/` | validated, unpacked immutable wheel trees keyed by wheel digest |
| `install/wheel_install_plan_cache.py` | `resolution-<interpreter>/` | short-lived exact-pin plan receipts referencing validated archives |
